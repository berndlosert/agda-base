...
consTree : {{_ : Measured v a}} -> a -> FingerTree v a -> FingerTree v a
consTree a Empty = Single a
consTree a (Single b) = deep (One a) Empty (One b)
consTree a (Deep s (Four b c d e) m sf) =
  Deep (measure a <> s) (Two a b) (consTree (node3 c d e) m) sf
...
snocTree : {{_ : Measured v a}} -> FingerTree v a -> a -> FingerTree v a
snocTree Empty a = Single a
snocTree (Single a) b = deep (One a) Empty (One b)
snocTree (Deep s pr m (Four a b c d)) e =
  Deep (s <> measure e) pr (snocTree m (node3 a b c)) (Two d e)
