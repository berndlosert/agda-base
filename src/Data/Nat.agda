...
-- For some odd reason, we have to define this in order for other modules
...
instance Div:Nat : Div Nat
Div:Nat = record {
    Constraint = λ { zero -> Void; (suc n) -> Unit };
    _/_ = λ { m (suc n) -> div-helper zero n m n }
  }

-- The mod operation for natural numbers.
open import Notation.Mod public
instance Mod:Nat : Mod Nat
Mod:Nat = record {
    Constraint = λ { zero -> Void; (suc n) -> Unit };
    _%_ = λ { m (suc n) -> mod-helper zero n m n }
  }

instance
  -- Nat is a semigroup with respect to +.
  open import Data.Semigroup
  Semigroup:Nat : Semigroup Nat
  Semigroup:Nat = Semigroup: _+_

  -- Nat is a monoid with respect to +.
