...
Coyoneda : (C : Category) -> (C => Sets) -> C => Sets
Coyoneda C F Y = exists \ X -> F X * hom C X Y

-- Coyoneda C F is a functor.
Functor:Coyoneda : (C : Category) (F : C => Sets)
...
lowerCoyoneda : {C : Category} {F : C => Sets} {{_ : Functor C Sets F}} ->
  let instance _ = Trans: C Sets in
  Coyoneda C F ~> F
lowerCoyoneda (_ , x , f) = map f x

liftCoyoneda : {C : Category} {F : C => Sets} ->
  let instance _ = Trans: C Sets in
  F ~> Coyoneda C F
...
foldCoyoneda : {C : Category} {F G : C => Sets} {{_ : Functor C Sets G}} ->
  let instance _ = Trans: C Sets in
  (F ~> G) -> Coyoneda C F ~> G
foldCoyoneda alpha (_ , x , f) = map f (alpha x)

-- This is the left adjunct.
ladjunctCoyoneda : {C : Category} {F G : C => Sets} {{_ : Functor C Sets G}} ->
  let instance _ = Trans: C Sets in
  (Coyoneda C F ~> G) -> (F ~> G) 
