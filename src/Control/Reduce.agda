...
reduce {t} {a} {b} (reducer {c} init step done) xs =
    foldr step1 done xs init
  where
    step1 : a -> (c -> b) -> c -> b
    step1 x k acc = case (step acc x) \ where
      (reduced true acc1) -> done acc1
      (reduced false acc1) -> k $! acc1
...
intoProduct : {{Multipliable a}} -> {{FromN 1 a}} -> Reducer a a
